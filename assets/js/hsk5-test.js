...
// ===== AUTO SET HSK5 LEVEL =====
function autoSetHSK5Level() {
    // Automatically set HSK5 level
    setSelectedLevel('hsk5', null, '5', 'hsk5_questions');
    console.log('HSK5 level auto-set');
}

// ===== LOAD HSK5 QUESTIONS =====
async function loadHSK5Questions() {
    try {
        const result = await getQuestionsByLevel('hsk5_questions');
        
        if (!result.success) {
            alert(`Đề thi HSK5 đang được cập nhật.`);
            return;
        }
        ...
        alert('Lỗi khi tải câu hỏi HSK5: ' + err.message);
    }
}
...
// ===== SETUP SUBMIT BUTTON =====
function setupHSK5SubmitButton() {
    ...
    if (btnSubmit) {
        btnSubmit.addEventListener('click', async function() {
            ...
            await submitHSK5Test();
        });
    }
}

// ===== SETUP FINISH BUTTON =====
function setupHSK5FinishButton() {
...
function setupHSK5NavigationButtons() {
    // Next section button
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('HSK5 test page loaded, initializing...');
    ...
    // Setup navigation buttons
    setupHSK5NavigationButtons();
    
    // Setup submit button
    setupHSK5SubmitButton();
    
    // Setup finish button
    setupHSK5FinishButton();
    
    console.log('HSK5 test initialization complete');
});
